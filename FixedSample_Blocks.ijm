//////////////////////////////////////////////////////////////////////////////////////////////////
//
// Name:	RegionsScanner_XYBlocks
// Author: 	Sebastien Tosi (IRB/ADMCF)
// Date:	09-11-2012	
//			   
// Similar to RegionsScanner but the wells (UV) are opened independently.
...
// Leica widefield config:     	xyFlip = 0 xsign = 1 ySign = 1
//
//////////////////////////////////////////////////////////////////////////////////////////////////
...
AnalysisFunctionsPath = getDirectory("macros")+"FixedCAMBlockAnalysisFunctions.ijm";
/////////////////////////////////////////////////////////////////////////////////////////////
...
Dialog.create("RegionsScanner_Blocks setup");
...
Dialog.addCheckbox("Micro-manager mode?", false);
...
Dialog.addNumber("Well start row    ", 0);
Dialog.addNumber("Well start column ", 0);
Dialog.addNumber("Well end row      ", 1);
Dialog.addNumber("Well end column   ", 1);
Dialog.show();
...
{
	File.append("/cli:frank /app:matrix /cmd:startscan\r", ExpPath+"CAMScript2.txt");
	File.append("/cli:frank /app:matrix /cmd:deletelist\r", ExpPath+"CAMScript2.txt");
}
for(i=0;i<tPos;i++)
{
	offx = -xSign*xList[i];
	offy = -ySign*yList[i];
	if(xyFlip==1)File.append("/cli:frank /app:matrix /cmd:add /tar:camlist /exp:"+JobHigh+" /ext:none /slide:0 /wellx:"+d2s(BlockXInd+1,0)+" /welly:"+d2s(BlockYInd+1,0)+" /fieldx:"+d2s(FieldxList[i],0)+" /fieldy:"+d2s(FieldyList[i],0)+" /dxpos:"+d2s(offy,0)+" /dypos:"+d2s(offx,0)+"\r", ExpPath+"CAMScript2.txt");
	else File.append("/cli:frank /app:matrix /cmd:add /tar:camlist /exp:"+JobHigh+" /ext:none /slide:0 /wellx:"+d2s(BlockXInd+1,0)+" /welly:"+d2s(BlockYInd+1,0)+" /fieldx:"+d2s(FieldxList[i],0)+" /fieldy:"+d2s(FieldyList[i],0)+" /dxpos:"+d2s(offx,0)+" /dypos:"+d2s(offy,0)+"\r", ExpPath+"CAMScript2.txt");
}
...
File.append("/cli:frank /app:matrix /cmd:startcamscan /runtime:9999 /repeattime:9999\r", ExpPath+"CAMScript2.txt");
if(Umanager == true)
{	
