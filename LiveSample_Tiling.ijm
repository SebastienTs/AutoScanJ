...
AnalysisFunctionsPath = getDirectory("macros")+"LiveCAMAnalysisFunctions.ijm";
OfflineFilesPath = "F:\\Nicolas\\MatrixExperiment-09-09-2011-5mins\\09_09_24h_scan"; // Only required for offline mode
JString = "J09";
OfflineZ = 3;
OfflineX = 3;
OfflineY = 2;
...
Dialog.addNumber("Wells: number of columns", 3);
Dialog.addNumber("Wells: number of rows", 2);
Dialog.addNumber("Maximum nb of repetitions for primary scan", 180);
Dialog.addNumber("Repetition period for primary scan (sec)", 5);
Dialog.addNumber("Nb of repetitions for secondary scan", 30);
...
{ 
	...
	LogWindowCloser();
	// Enable all wells
	for(i=1;i<=NbWellsX;i++)for(j=1;j<=NbWellsY;j++)print("/cli:frank /app:matrix /cmd:enable /slide:0 /wellx:"+d2s(i,0)+" /welly:"+d2s(j,0)+" /fieldx:0 /fieldy:0 /value:true");	
	...
	if(selectionType!=-1)
	{
		...
		LogWindowCloser();
		for(i=1;i<=NbWellsX;i++)for(j=1;j<=NbWellsY;j++)print("/cli:frank /app:matrix /cmd:enable /slide:0 /wellx:"+d2s(i,0)+" /welly:"+d2s(j,0)+" /fieldx:0 /fieldy:0 /value:false");
		for(i=0;i<tPos;i++)print("/cli:frank /app:matrix /cmd:enable /slide:0 /wellx:"+d2s(WellxList[i],0)+" /welly:"+d2s(WellyList[i],0)+" /fieldx:0 /fieldy:0 /value:true");
		print("/cli:frank /app:matrix /cmd:deletelist");
		for(i=0;i<tPos;i++)
		{
			offx = -xSign*xList[i];
			offy = -ySign*yList[i];
			if(xyFlip==1)print("/cli:frank /app:matrix /cmd:add /tar:camlist /exp:"+JobHigh+" /ext:none /slide:0 /wellx:"+d2s(WellxList[i],0)+" /welly:"+d2s(WellyList[i],0)+" /fieldx:1 /fieldy:1 /dxpos:"+d2s(offy,0)+" /dypos:"+d2s(offx,0));
			else print("/cli:frank /app:matrix /cmd:add /tar:camlist /exp:"+JobHigh+" /ext:none /slide:0 /wellx:"+d2s(WellxList[i],0)+" /welly:"+d2s(WellyList[i],0)+" /fieldx:1 /fieldy:1 /dxpos:"+d2s(offx,0)+" /dypos:"+d2s(offy,0));
		}
		print("/cli:frank /app:matrix /cmd:startscan");
